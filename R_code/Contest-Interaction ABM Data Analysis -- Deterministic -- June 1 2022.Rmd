---
title: "Contest-Interaction ABM Data Analysis -- Deterministic"
...
# All data was generated in NetLogo Agent-Based Models (ABM) and imported into this program for analysis. These data and analyses are used in the publication "Energetic skew varies with ecology, but not with dominance style: findings from an agent-based model" by Marcy S. Ekanayake-Weber, Christopher J. O'Connor-Coates, and Andreas Koenig.

# The NetLogo model was created by Marcy S. Ekanayake-Weber and Christopher J. O'Connor-Coates. The following R-code was written by Christopher J. O'Connor-Coates. For questions regarding the R-code email Christopher J. O'Connor-Coates at christopher.oconnor-coates@stonybrook.edu.
...
# Average slopes across all trials (currently unused in the analysis):
...
# Generate interaction matrices and calculate summary statistics for each trial:
mut1<-data.frame(X1=test[[1]],X2=test[[2]],Victories=test[[3]])
mat1<-inter.mat(mut1)
tri1<-tr.tri(mat1)
dci1<-DCindex(mat1)
...
# Summarize nummber of vistories in each trial:
int1<-sum(mut1$Victories)
int2<-sum(mut2$Victories)
int3<-sum(mut3$Victories)
int4<-sum(mut4$Victories)
...
# Summarize number of avoids in each trial:
av1<-sum(a1$Victories)
av2<-sum(a2$Victories)
av3<-sum(a3$Victories)
av4<-sum(a4$Victories)
...
# Average number of avoids across trials:
avg.av.ks.dc<-(sum(av1, av2, av3, av4, av5, av6, av7, av8, av9, av10))/10

# Average number of victories across trials
avg.int.ks.dc<-(sum(int1, int2, int3, int4, int5, int6, int7, int8, int9, int10))/10

# Calculate linear models and slopes for each individual trial:
...
# Generate data frame of summary statistics calculated for each trial:
...
# Write .csv documents of each interaction matrix for each trial:
...
# Generate interaction matrices and summary statistic data for each trial:
mut1<-data.frame(X1=test[[1]],X2=test[[2]],Victories=test[[3]])
mat1<-inter.mat(mut1)
tri1<-tr.tri(mat1)
dci1<-DCindex(mat1)
...
# Summarize the number of victories in each trial:
int1<-sum(mut1$Victories)
int2<-sum(mut2$Victories)
int3<-sum(mut3$Victories)
int4<-sum(mut4$Victories)
...
# Summarize the number of avoids in each trial:
av1<-sum(a1$Victories)
av2<-sum(a2$Victories)
av3<-sum(a3$Victories)
av4<-sum(a4$Victories)
...
# Average number of avoids across trial:
avg.av.ko.dc<-(sum(av1, av2, av3, av4, av5, av6, av7, av8, av9, av10))/10

# Averag number of victories across trials
...
# Generate data frame of summary statistics for all trials:
...
# Write .csv files for each interaction matrix for each trial:
...
# Generate data frame of summary statistics for each trial:
...
# Generate data frame of summary statistics across trials
...
# Write .csv files of each interaction matrix for each trial:
...
# Generate linear models of energy over time and David scores for each type of interaction:
...
overall.clump.deter$combo2<-paste(overall.clump.deter$strategy,overall.clump.deter$variable) #final dataframe for deterministic clumped data
```
...
```
```{r Attacks Uniform Dterministic Data Frame}
#knowledge mutual
...
## Overall Data Frames
```{r Full Overall Deterministic Data Frame}
# Generate single data frame for all of the deterministic data calculated in the previous chunks:

overall.ss.deter=rbind(overall.clump.deter,overall.unif.deter) #final overall data frame for all deterministic data
...
write.csv(overall.deter.data,"~/Desktop/deterministic_data_frame.csv",row.names=F) #.csv document of overall deterministic data frame for publication
```
```{r Full Clumped Deterministic Avoid Data Frame}
...
write.csv(overall.deter.clumped.avoid.data,"~/Desktop/avoid_deterministic_data_frame.csv",row.names=F) #.csv document of avoids deterministic data frame for publication
```
```{r Full Clumped Deterministic Attacks Data Frame}
...
write.csv(overall.deter.clumped.attacks.data,"~/Desktop/attacks_deterministic_data_frame.csv",row.names=F) #.csv document of attacks deterministic data frame for publication
```
```{r Full Uniform Deterministic Avoid Data Frame}
...
write.csv(overall.deter.uniform.avoid.data,"~/Desktop/avoid_deterministic_data_frame.csv",row.names=F) #.csv document of avoid deterministic data frame for publication

overall.ss.deter.avoid<-overall.ss.deter.uniform.avoid
```
```{r Full Uniform Deterministic Attacks Data Frame}
# Generate single data frame for the uniform avoids data calculated in the previous chunks:
...
write.csv(overall.deter.uniform.attacks.data,"~/Desktop/attacks_deterministic_data_frame.csv",row.names=F) #.csv document of attacks deterministic data frame for publication

overall.ss.deter.attacks<-overall.ss.deter.uniform.attacks
```
...
# Generate summary tables of the deterministic uinform data for publication:
...
#write.csv(total.ave,"~/Desktop/averages_table.csv")
#write.csv(overall.ss.deter.clumped.avoid,"~/Desktop/overall_deterministic_table.csv")
#write.csv(overall.ss.prob,"~/Desktop/overall_probabilistic_table.csv")
```
...
#write.csv(total.ave,"~/Desktop/averages_table.csv")
#write.csv(overall.ss.deter.clumped.attacks,"~/Desktop/overall_deterministic_table.csv")
#write.csv(overall.ss.prob,"~/Desktop/overall_probabilistic_table.csv")
```
...
#write.csv(total.ave,"~/Desktop/averages_table.csv")
#write.csv(overall.ss.deter.uniform.avoid,"~/Desktop/overall_deterministic_table.csv")
#write.csv(overall.ss.prob,"~/Desktop/overall_probabilistic_table.csv")
...
#write.csv(total.ave,"~/Desktop/averages_table.csv")
#write.csv(overall.ss.deter.uniform.attacks,"~/Desktop/overall_deterministic_table.csv")
#write.csv(overall.ss.prob,"~/Desktop/overall_probabilistic_table.csv")
```

## Energy Averages Summary Table For Publication
````{r Energy Averages Data Frame}
# Generate data frame for all energy data from clumped scendarios:
library(zoo)
library(dplyr)
n <- 100
...
## Corrected Attack Proportions Data
```{r Corrected Attack Proportions}
# The original attack proportion (escalation) data was not recorded correctly in NetLogo and had to be recollected. The below code organizes and analyses these correct data.
...
# Organize the attack proportion data and place into data frame in order by trial:
...
# Label each set of 10 trials by the Who variable (mutia, opponent, self) and the What variable (History, RHP):
